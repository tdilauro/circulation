...
from palace.manager.celery.tasks.reports import JOB_KEY_MAPPING
...
class TestReportController:
    ...
    @patch("palace.manager.api.admin.controller.report.generate_report_task")
    @patch(
        "palace.manager.api.admin.controller.report.generate_inventory_and_hold_reports"
    )
    def test_generate_report_authorization(
        ...
        valid_report_key = next(iter(JOB_KEY_MAPPING))
        ...
        with flask_app_fixture.test_request_context(
            ...
            assert_and_clear_caplog(generate_inventory_report(), sysadmin_email)
            assert_and_clear_caplog(generate_report(), sysadmin_email)
        ...
        with flask_app_fixture.test_request_context(
            ...
            assert_and_clear_caplog(generate_inventory_report(), librarian_email)
            assert_and_clear_caplog(generate_report(), librarian_email)
    ...
    def test_generate_report_celery_task(
        ...
        system_admin, _ = create(db.session, Admin, email=email_address)
        system_admin.add_role(AdminRole.SYSTEM_ADMIN)
        test_key = next(iter(JOB_KEY_MAPPING))
    ...
    def test_generate_report(
        ...
        with flask_app_fixture.test_request_context(
            ...
            for report_key, report_info in JOB_KEY_MAPPING.items():
                response = controller.generate_report(report_key=report_key)
                assert response.status_code == HTTPStatus.ACCEPTED
                assert isinstance(response, Response)
                assert (
                    "The completed reports will be sent to"
                    in response.get_json().get("message")
                )
                assert sysadmin.email in response.get_json().get("message")
                assert log_message_suffix in caplog.text
                assert report_info.JOB_TITLE in caplog.text
                caplog.clear()
    ...
    def test_generate_report_invalid_key(
        ...
        with flask_app_fixture.test_request_context(
            ...
            assert exc.value.problem_detail.title == INVALID_REPORT_JOB_KEY.title
            assert exc.value.problem_detail == INVALID_REPORT_JOB_KEY.detailed(
                f"No currently defined report job is associated with the specified key (key='{invalid_key}')."
            )
    ...
    def test_generate_report_exception(
        ...
        valid_key = next(iter(JOB_KEY_MAPPING))
